...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    rider_phone = serializers.CharField(source='rider.phone_number', read_only=True, allow_null=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'amount', 'order', 'order_id', 'warehouse', 'warehouse_name',
            'payer', 'payer_phone', 'payee', 'payee_phone',
            'rider', 'rider_phone', 'transaction_id', 'distance_km',
            'payment_method', 'notes', 'created_at', 'updated_at', 'completed_at'
        ]
...
class CreateShopkeeperPaymentSerializer(serializers.ModelSerializer):
    ...
    def validate_order(self, value):
        ...
        if value.status not in ['accepted', 'in_transit', 'delivered']:
            raise serializers.ValidationError("Cannot make payment for orders that are not accepted.")
        return value
...
class CreateRiderPayoutSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if order.status != 'delivered':
            raise serializers.ValidationError("Payout can only be made for delivered orders.")
        ...
        if existing_payout:
            raise serializers.ValidationError("Payout already exists for this rider and order.")
